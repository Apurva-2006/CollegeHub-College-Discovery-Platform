    ...
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:border-gray-200 transition-all p-5 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-2 text-center mt-auto pt-2">
        ...
        <div className="bg-gray-50 rounded-xl py-2 px-1">
          ...
          <p className="text-sm font-bold text-[#6D28D9]">{bestNirf !== 999 ? `#${bestNirf}` : "—"}</p>
        </div>
        <div className="bg-gray-50 rounded-xl py-2 px-1">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          ...
          className={cn(
            ...
            isSaved ? "text-[#FB7185] bg-rose-50 cursor-default" : "text-gray-300 hover:text-[#FB7185] hover:bg-rose-50"
          )}
        >
...
function SavedPageInner() {
  // Declare router FIRST — before any effect that uses it
  const router = useRouter();
  ...
  // Derive the fetch IDs; if empty, skip fetch and set empty map directly via render
  ...
  // Derive empty collegeMap during render when allIds is empty (avoids setState-in-effect)
  const resolvedCollegeMap = allIds.length === 0 ? new Map<string, College>() : collegeMap;
  ...
  const handleQuickSave = (ids: string[]) => {
    const names = ids.map((id) => resolvedCollegeMap.get(id)?.shortName).filter(Boolean) as string[];
    const defaultLabel = names.length > 0 ? names.join(" vs ") : `${ids.length} colleges`;
    setPendingSave({ ids, label: defaultLabel });
  };

  const savedColleges = savedIds.map((id) => resolvedCollegeMap.get(id)).filter(Boolean) as College[];
  const recentlyViewedColleges = viewedIds.map((id) => resolvedCollegeMap.get(id)).filter(Boolean) as College[];
  ...
  const comparisonMatches = (label: string, ids: string[]) => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    if (label.toLowerCase().includes(q)) return true;
    return ids.some((id) => {
      const c = resolvedCollegeMap.get(id);
        ...
        <div className="min-h-[400px]">
          {tab === "saved-colleges" && (
            <section>
              ...
              {loading ? renderSkeletonGrid() : savedColleges.length === 0 ? (
                <EmptySavedColleges />
          ...
          {tab === "recently-viewed" && (
            <section>
              ...
              {loading ? renderSkeletonGrid() : recentlyViewedColleges.length === 0 ? (
                <EmptyRecentlyViewed />
          ...
          {tab === "saved-comparisons" && (
            <section>
              ...
              {comparisons.length === 0 ? <EmptySavedComparisons /> : filteredComparisons.length === 0 ? (
                <NoSearchResults query={searchQuery} onClear={clearSearch} />
              ) : (
                  ...
                  {[...filteredComparisons]
                    ...
                    .map((comp) => (
                      <ComparisonSetCard
                        ...
                        allColleges={resolvedCollegeMap}
          ...
          {tab === "recent-comparisons" && (
            <section>
              ...
              ) : recentComparisons.length === 0 ? <EmptyRecentComparisons /> : filteredRecentComparisons.length === 0 ? (
                <NoSearchResults query={searchQuery} onClear={clearSearch} />
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
