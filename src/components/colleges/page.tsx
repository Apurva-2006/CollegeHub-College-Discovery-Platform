...
function countActiveFilters(f: CollegeFilters): number {
  return (
    f.states.length + f.cities.length + f.feeRange.length + f.ratings.length +
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {loading ? "Loading..." : `${total} of ${total} colleges`}
          </p>
        </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative ml-auto w-80 max-w-full h-full bg-white overflow-y-auto shadow-xl">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between z-10">
              ...
              <button onClick={() => setMobileSidebarOpen(false)} className="text-gray-500 hover:text-gray-800">✕</button>
            </div>
            <div className="p-4">
              <FilterSidebar filters={filters} onChange={setFilters} activeCount={activeCount} />
            </div>
