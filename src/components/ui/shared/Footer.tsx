    ...
    <footer className="bg-white border-t border-gray-100 mt-auto">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              {/* Twitter */}
              <a href="#" aria-label="Twitter" className="text-gray-400 hover:text-[#6D28D9] transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              {/* LinkedIn */}
              <a href="#" aria-label="LinkedIn" className="text-gray-400 hover:text-[#6D28D9] transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              {/* Instagram */}
              <a href="#" aria-label="Instagram" className="text-gray-400 hover:text-[#6D28D9] transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              ...
              {/* GitHub */}
              <a href="#" aria-label="GitHub" className="text-gray-400 hover:text-[#6D28D9] transition-colors">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {EXPLORE.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-sm text-gray-500 hover:text-[#6D28D9] transition-colors">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Streams */}
          <div>
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Streams</h3>
            <ul className="space-y-2.5">
              {STREAMS.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-sm text-gray-500 hover:text-[#6D28D9] transition-colors">
        ...
        <div className="mt-10 pt-6 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs text-gray-400">© 2025 CollegeHub. Crafted for students, by students.</p>
          <p className="text-xs text-gray-400 flex items-center gap-1">
            Made with <span className="text-red-500 text-sm">♥</span> in India
          </p>
        </div>
      </div>
    </footer>
